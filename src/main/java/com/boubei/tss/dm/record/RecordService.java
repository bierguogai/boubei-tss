...
public interface RecordService {
    ...
    @Logable(operateObject="录入表", operateInfo="<#if args[1]=1>停用<#else>启用</#if>了报表(ID = ${args[0]?default(\"\")}) ")
    ...
    @Logable(operateObject="录入表", operateInfo="(ID: ${args[0]})节点移动到了(ID: ${args[1]})节点<#if args[2]=1>之下<#else>之上</#if>")
    void sort(Long startId, Long targetId, int direction);
 
    @Logable(operateObject="录入表", operateInfo="移动(ID: ${args[0]}) 节点至 (ID: ${args[1]}) 组下。")
