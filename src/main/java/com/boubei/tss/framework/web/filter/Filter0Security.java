...
public class Filter0Security implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        if(referer != null && !referer.contains(serverName)) { // 如果是跨域访问了，则过滤ip白名单
        	...
        	String ipWhiteListConfig = ParamConfig.getAttribute(PX.IP_WHITE_LIST);
        ...
        // 检测权限（判断用户是否登录，此时Filter3Context还没有执行，无法使用Environment）
    ...
    private boolean isNeedPermission(String servletPath, HttpServletRequest request) {
    	...
    	// 2、检查URL白名单，白名单内的，放行
    	String whiteListConfig = ParamConfig.getAttribute(PX.URL_WHITE_LIST);
    	List<String> whiteList = new ArrayList<String>();
