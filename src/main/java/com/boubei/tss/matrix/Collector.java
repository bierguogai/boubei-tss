...
/**
 * 定时收集使用情况：创建多少报表、数据表资源、登陆次数、登录用户数、分时段访问统计、异常信息等，
 * 
 * 集中发往boubei.com，方式：
 * 1、前端 JSONP
...
select name, createTime ct, updateTime ut, lockVersion, rctable, define, datasource, customizejs, customizepage, customizetj, 
 ...
 from dm_record where type = 1 and updateTime > curdate()
	...
	/**
	 * 收集操作日志
SELECT id, operatetable, operationcode, operatetime, operatorip, operatorbrowser, operatorname, content
 FROM tssbi.component_log
 WHERE operateTable IN ('数据表','报表','站点栏目','门户结构','门户组件','站点栏目','文章','角色','用户','用户登录','系统参数','系统异常')
